...
# ============================================================
# OMNI CAVE - FIXED VERSION
# ============================================================
# Environment variables:
# DISCORD_TOKEN       = Discord bot token  (also accepts YOUR_TOKEN_HERE)
...
if not TOKEN:
    raise SystemExit(
        "ERROR: Set DISCORD_TOKEN (or YOUR_TOKEN_HERE) in environment variables."
    )
...
def get_settings(guild_id):
    ...
    with db:
        db.execute(
            "INSERT OR IGNORE INTO settings (guild_id, prefix) VALUES (?, ?)",
...
def get_guild_prefix(guild_id) -> str:
    if not guild_id:
...
def set_guild_prefix(guild_id, prefix: str) -> str:
    prefix = prefix.strip()
...
async def dynamic_prefix(bot_instance, message: discord.Message):
...
# ============================================================
def xp_needed(level):
    # Faster curve than 100 + level*75
    return 70 + (level * 45)
...
async def on_ready():
    ...
    await bot.change_presence(
        activity=discord.Game(name="Omni Cave • /help or !help")
    )
...
async def on_member_remove(member: discord.Member):
    ...
    settings = get_settings(member.guild.id)
    channel = (
        member.guild.get_channel(settings["goodbye_channel"])
...
async def on_message(message: discord.Message):
    ...
    # Prefix commands must run first, before any early return.
    await bot.process_commands(message)

    if not message.guild:
        return

    # Skip XP / auto-chat when the message is a bot command
    prefix = get_guild_prefix(message.guild.id)
    content = message.content or ""
    is_command = content.startswith(prefix) or (
        bot.user and bot.user.mentioned_in(message)
    )

    # XP — 15s cooldown, 20-40 XP, extra for longer messages
    ...
    if now - xp_cooldowns.get(key, 0) >= XP_COOLDOWN_SECONDS:
        ...
        extra = min(len(content.strip()) // 40, 10)
        amount += extra
    ...
    if mentioned:
        ...
        if not prompt:
            ...
            await message.reply(
                f"Hey! I'm here. Use `/ai` or `{current}ai` to ask me something."
            )
            return
    ...
    # Auto-chat: env channel list OR /chat on for this channel
    channel_allowed = False
    if CHAT_CHANNEL_IDS and message.channel.id in CHAT_CHANNEL_IDS:
        channel_allowed = True
    if chat_enabled.get(message.channel.id):
        channel_allowed = True
...
async def on_command_error(ctx: commands.Context, error: commands.CommandError):
    ...
    if isinstance(error, commands.BadArgument):
        await ctx.send("Invalid argument. Check the command and try again.")
        return
...
@bot.command(name="setprefix", aliases=["prefixset", "changeprefix"])
@commands.guild_only()
@commands.has_permissions(manage_guild=True)
async def setprefix_cmd(ctx: commands.Context, new_prefix: str):
    """Change this server's command prefix. Example: !setprefix ?"""
    if not valid_prefix(new_prefix):
        await ctx.send(
            "Prefix must be 1-5 characters, no spaces, and cannot start with `/`."
        )
        return

    saved = set_guild_prefix(ctx.guild.id, new_prefix)
    await ctx.send(
        f"Prefix updated. New prefix is `{saved}`\n"
        f"Example: `{saved}ping`  `{saved}help`  `{saved}setprefix !`"
    )


@bot.command(name="prefix")
@commands.guild_only()
async def prefix_cmd(ctx: commands.Context):
    """Show the current prefix for this server."""
    current = get_guild_prefix(ctx.guild.id)
    await ctx.send(
        f"Current prefix is `{current}`\n"
        f"Change it with `{current}setprefix <new>` or `/setprefix`."
    )
...
async def help_prefix(ctx: commands.Context):
    current = get_guild_prefix(ctx.guild.id) if ctx.guild else DEFAULT_PREFIX
    embed = discord.Embed(
        title="Omni Cave Commands",
        description=(
            f"Prefix in this server: `{current}`\n"
            f"Slash commands still work with `/`."
        ),
        color=discord.Color.blurple(),
    )
    embed.add_field(
        name="Prefix",
        value=f"`{current}prefix` `{current}setprefix`",
        inline=False,
    )
    embed.add_field(
        name="Useful prefix commands",
        value=f"`{current}ping` `{current}help` `{current}level` `{current}leaderboard`",
        inline=False,
    )
    embed.add_field(
        name="Moderation",
        value="`/warn` `/warnings` `/clearwarnings` `/mute` `/ban` `/unban` `/role` `/kick` `/clear`",
        inline=False,
    )
    embed.add_field(
        name="AI & Fun",
        value="`/ai` `/wave` `/8ball` `/rps` `/slots`",
        inline=False,
    )
    embed.add_field(
        name="Leveling",
...
@commands.guild_only()
async def level_prefix(ctx: commands.Context, member: discord.Member | None = None):
...
# ============================================================
@bot.tree.command(name="setprefix", description="Change this server's text command prefix")
@app_commands.checks.has_permissions(manage_guild=True)
@app_commands.describe(new_prefix="New prefix, for example ! or ? or .")
async def setprefix_slash(interaction: discord.Interaction, new_prefix: str):
    ...
    if not valid_prefix(new_prefix):
        ...
        return
    saved = set_guild_prefix(interaction.guild.id, new_prefix)
    await interaction.response.send_message(
        f"Prefix updated. New prefix is `{saved}`\n"
        f"Example: `{saved}ping`  `{saved}help`"
    )


@bot.tree.command(name="prefix", description="Show this server's text command prefix")
async def prefix_slash(interaction: discord.Interaction):
    current = get_guild_prefix(interaction.guild.id) if interaction.guild else DEFAULT_PREFIX
    await interaction.response.send_message(
        f"Current prefix is `{current}`\n"
        f"Change it with `/setprefix` or `{current}setprefix <new>`."
    )
...
async def help_command(interaction: discord.Interaction):
    embed = discord.Embed(
        title="🤖 Omni Cave Commands",
        description="Slash commands use `/`. Text commands use this server's prefix (`!` by default, change with `/setprefix`).",
        color=discord.Color.blurple(),
    )
    embed.add_field(
        name="🛡️ Moderation",
    ...
    embed.add_field(
        name="📈 Leveling",
        value="`/level` `/leaderboard` `/prefix` `/setprefix`",
        inline=False,
    )
    embed.add_field(
        name="👋 Server",
